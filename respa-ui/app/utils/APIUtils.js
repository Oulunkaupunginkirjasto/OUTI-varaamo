...
function getHeadersCreator(headers) {
  return (state) => {
    ...
    if (state.client.clientKey) {
      authorizationHeaders['Varaamo-Client-Key'] = state.client.clientKey;
    }
    if (state.ui.locale.locale) {
      authorizationHeaders['Accept-Language'] = state.ui.locale.locale;
    }
...
function getSuccessPayload(options) {
  return (action, state, response) => {
    const jsonPromise = getJSON(response);
    if (options.loadNext && options.requestFunction) {
      jsonPromise.then((json) => {
        if (json.next) {
          console.log('will get next'); // eslint-disable-line
          options.requestFunction({ page: 2 }, { loadNext: options.loadNext });
        }
      });
    }
    return jsonPromise.then(createTransformFunction(options.schema));
  };
}
