        ...
        if not resource.is_admin(request_user):
            ...
            if reservable_before and data['begin'] >= reservable_before:
                raise ValidationError(_('The resource is reservable only before %(datetime)s') %
                                        {'datetime': localtime(reservable_before, resource.unit.get_tz())})
            ...
            if reservable_after and data['begin'] < reservable_after:
                raise ValidationError(_('The resource is reservable only after %(datetime)s') %
                                        {'datetime': localtime(reservable_after, resource.unit.get_tz())})
...
class ReservationViewSet(munigeo_api.GeoModelAPIView, viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        request_language = get_language_from_request(self.request, check_path=False)
        user_language = self.request.user.get_preferred_language()
        if request_language and user_language != request_language:
            self.request.user.preferred_language = request_language
