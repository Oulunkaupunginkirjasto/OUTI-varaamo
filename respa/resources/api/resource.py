...
from resources.pagination import PurposePagination, ResourcePagination
...
class PurposeSerializer(TranslatedModelSerializer):
    ...
    class Meta:
        model = Purpose
        fields = ['name', 'description', 'parent', 'id', 'display_order']
...
class ResourceSerializer(TranslatedModelSerializer, munigeo_api.GeoModelSerializer):
    ...
    reservable_after_days = serializers.ReadOnlyField(source='get_reservable_after_days')
    ...
    def get_queryset(self):
        if self.request.user.is_staff:
            ...
            if self.request.client_configuration:
                config = self.request.client_configuration
                if config.units:
                    qs = qs.filter(unit__in=config.units.all())
                if config.purposes:
                    qs = qs.filter(purposes__in=config.purposes.all())
            return qs
