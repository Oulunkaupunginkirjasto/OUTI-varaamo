...
def to_weeks(periods):
    """
    Combine individual days into weeks
    :param periods: list of day schedules in ascending date order
    :return: Generator
    """
    week = {}
    try:
...
def process_periods(data, unit):
    ...
    weeks = to_weeks(periods)

    def get_date(data):
        return data.get('date', None)

    for week in weeks:
        week_start = min(week.values(), key=get_date).get('date')
        week_end = max(week.values(), key=get_date).get('date')
        description = list(week.values())[0].get('description')
        closed = all(map(lambda x: not x or x['closed'], week.values()))
        name = 'imported {}-{}'.format(week_start, week_end)
        ...
        for key in sorted(week):
            day = week[key]
            nper.days.create(weekday=int(day.get('day')) - 1,
                             opens=day.get('opens'),
                             closes=day.get('closes'),
