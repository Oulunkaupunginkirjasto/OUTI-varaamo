...
def send_respa_mail(email_address, subject, template_name, context, language=DEFAULT_LANG):
    ...
    with translation.override(language):
        ...
        try:
            ...
            logger.warning("Failed to send email: {}".format(str(e)))
...
def create_reservable_before_datetime(days_from_now, tz=None):
    if tz is None:
        tz = pytz.UTC
    ...
    dt = localtime(dt, pytz.UTC)

    return dt


def create_reservable_after_datetime(days_from_now, tz=None):
    if tz is None:
        tz = pytz.UTC
    if days_from_now is None:
        return None
