...
REQUIRED_RESERVATION_EXTRA_FIELDS = ('reserver_name', 'reserver_phone_number', 'event_description',
...
class Reservation(ModifiableModel):
    ...
    def __str__(self):
        tz = self.resource.unit.time_zone if self.resource and self.resource.unit else pytz.UTC
        begin = self.get_begin_tz(tz)
        end = self.get_end_tz(tz)
        return "%s -> %s: %s" % (begin, end, self.resource)
    ...
    def send_reservation_mail(self, subject, template_name, extra_context=None, user=None):
        ...
        if user:
            ...
            if not (self.reserver_email_address or self.user):
                logger.info("Attempted to send an email without an email address.")
                return
            email_address = self.reserver_email_address or self.user.email
            user = self.user
        if not email_address:
            logger.info("Attempted to send an email to user {} without an email address.".format(str(user)))
            return
